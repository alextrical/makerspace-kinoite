...
* This example allows to connect to the target through CAN Bootloader interface and execute some operations to the internal STM32 Flash memory. \n
*		- Connect to the device via CAN interface.
*		- File programming + verification step.
*		- Read 64 bytes from 0x08000000.
*		- Option bytes programming : BOR level.
*		- Display option bytes.
*.
* Go to the source code : \ref CAN_Bootloader 
* \example CAN_Bootloader
* This example allows to connect to the target through CAN Bootloader interface and execute some operations to the internal STM32 Flash memory. \n
*		- Connect to the device via CAN interface.
*		- File programming + verification step.
*		- Read 64 bytes from 0x08000000.
...
int CAN_Example(void) {
	...
	/* Target connect */
	int canConnectFlag = connectCanBootloader(canParam);
	if (canConnectFlag != 0) {
    ...
    unsigned char* dataStruct = nullptr;
